...
export const schema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (startIsMissing) {
      ...
      // Only check if start date is in the past if it's a new time slot (no timeSlotId) - for edits we allow past start dates since they may be existing slots that started in the past
