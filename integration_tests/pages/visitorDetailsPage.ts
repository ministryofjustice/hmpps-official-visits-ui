...
export default class VisitorDetailsPage extends AbstractPage {
  ...
  private constructor(page: Page) {
    super(page)
    this.header = page.locator('h1', { hasText: `Further visitor details (optional)` })
    this.caption = page.locator('.govuk-hint', { hasText: 'Book an official visit' })
  }

  static async verifyOnPage(superPage: Page): Promise<VisitorDetailsPage> {
    const page = new VisitorDetailsPage(superPage)
  ...
  async fillBoxForContact(index: number, notes: string) {
    await this.page.locator(`#visitorDetails\\[${index}\\]\\[notes\\]`).fill(notes)
  }
}
